...
oj.visualization.OrgChart = (function () {
    ...
    var Node = function (inputObj) {
        ...
        inputObj.orgChart.userData[index][5]

        this.cast = inputObj["cast"] || undefined;
        ...
        var getHTMLContent = function () {
            ...
            for (var i = 0; i < this.dimensionMatrix.length; i++) {
                ...
                for (var j = 0; j < this.dimensionMatrix[i].length; j++) {
                    ...
                    today = dd + '-' + mm + '-' + yyyy;
                    var apply = new Date(this.data) > new Date(today) ? 'none' : 'red';
                    if (i == 1 && j == 1) {
                        var br = `<div class="card"  style="background-color : ${apply}">
                         
                        <div  class="container" style="max-height: 145px;  " >
                          <h4><b>${this.name} </b></h4>
                          <hr/>
                          ${this.tooltip.length >0 ?`
                          <h4><b>Ticket Owner </b></h4>
                          <p> ${this.tooltip}</p>
                          <h4><b>${this.data} </b></h4>
                            <hr/>
                          `: "" }
